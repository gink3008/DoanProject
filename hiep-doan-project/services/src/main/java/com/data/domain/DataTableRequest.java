...
public class DataTableRequest<T> {
    ...
    private void prepareDataTableRequest(HttpServletRequest request) {
        ...
        if (parameterNames.hasMoreElements()) {
            ...
            if (!CommonUtil.isEmpty(this.getSearch())) {
            ...
            for (int i = 0; i < maxParamsToCheck; i++) {
                if (null != request.getParameter("columns[" + i + "][data]")
                        ...
                        && !CommonUtil.isEmpty(request.getParameter("columns[" + i + "][data]"))) {
                    ...
                    if (!CommonUtil.isEmpty(colSpec.getSearch())) {
    ...
    public PaginationCriteria getPaginationRequest() {
        ...
        SortBy sortBy = null;
        if (!CommonUtil.isEmpty(this.getOrder())) {
            sortBy = new SortBy();
        ...
        if (!CommonUtil.isNullOrEmpty(this.getColumns())) {
            for (DataTableColumnSpecs colSpec : this.getColumns()) {
                if (colSpec.isSearchable()) {
                    if (!CommonUtil.isEmpty(this.getSearch()) || !CommonUtil.isEmpty(colSpec.getSearch())) {
